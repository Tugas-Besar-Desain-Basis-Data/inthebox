                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th>Quantity</th>
                                    <th>Total</th>
                                    <th class="p-name">Status Pembayaran</th>
                                    <th>No Resi</th>
                                    <th class="p-name">Status Barang</th>
                                </tr>
                            </thead>
                            <?php
                                ...
                                $chart_cek = mysqli_query($koneksi,"select * from co_user where username='$user'");
                                $total_chart = 0;
                                $a=0;
                                $check_notes = mysqli_query($koneksi,"select * from co_user where username='$user'");
                                $notes = mysqli_fetch_assoc($check_notes);
                                while($chart = mysqli_fetch_array($chart_cek)){
                                    $a++;
                                    if($chart["status_barang"]!="Pesanan Telah Selesai"){
                                    $number_chart++;
                                    $id_chart_detail = $chart['id_detail'];
                                    $id_chart_barang = $chart['id_barang'];
                                    $jumlah_barang = $chart['jumlah'];
                                    $kode_bayar = $chart['resi'];

                                    $update_resi = $chart['resi_barang'];

                                    $status_ket = $chart['status'];

                                    $stat_barang = $chart['status_barang'];
                                    ...
                                    <td class="qua-col first-row">
                                        <div class="quantity">
                                            <div>
                                                ...
                                                $data_s = "SELECT * FROM detail_barang WHERE id='$id_chart_detail'";
                                                ...
                                                ?>
                                                    <input type="text" disabled name="jumlah" class="pro-qty" value=<?php echo $jumlah_barang ?>>
                                                <form method="POST" action="update.php">
                                                    <input type="hidden" name="resi" value="<?php echo $kode_bayar ?>" >
                                            </div>
                                        </div>
                                    </td>
                                    <td class="total-price first-row"><?php echo "Rp ". number_format($total_barang, 0, ".", "."). "" ?></td>
                                    <td class="total-price first-row"><?php echo $chart['status'] ?></td>
                                    <td class="total-price first-row"><?php echo $chart['resi_barang'] ?></td>
                                    <td class="total-price first-row"><?php echo $chart['status_barang'] ?></td>
                                </tr>
                            </tbody>
                            <?php
                                }}}}
                    ...
                    <div class="row">
                        <div class="col-lg-4">
                            ...
                            <div class="discount-coupon">
                                <h6>NOTES</h6>
                                <?php
                                    if($status_ket=="pending"){
                                ?>
                                    <h7>Lakukan Pembayaran Secepatnya Untuk Memproses Barang Anda</h7>
                                <?php
                                }
                                else if($status_ket=="verified") {
                                ?>
                                <h7>Lihat Status Barang Anda Diatas. Happy Shopping and Enjoy</h7>
                                <?php
                                }
                                ?>
                            </div>
                        ...
                        <div class="col-lg-4 offset-lg-4">
                            <div class="proceed-checkout">
                                <ul>
                                    <li class="subtotal">Kode Bayar :  <span></span></li>
                                    <li class="subtotal"><?php echo $kode_bayar ?></li>
                                    <li class="cart-total">Total <span><?php echo "Rp ". number_format($total_chart, 0, ".", "."). "" ?></span></li>
                                </ul>
                                <?php
                                    if($status_ket=="pending"){
                                ?>
                                    <input type="submit" class="proceed-btn" value=" KLIK UNTUK MEMVERIFIKASI PEMBAYARAN "></input>
                                <?php
                                    }
                                    else{
                                        if(($status_ket=="verified")&&($stat_barang=="Sedang Dikemas")){
                                ?>
                                    <input type="submit" class="proceed-btn" name="submit" value=" KLIK UNTUK MEREFRESH STATUS DIATAS "></input>
                                <?php
                                    }
                                        else if(($status_ket=="verified")&&($stat_barang=="Proses Pengiriman")){
                                ?>
                                        <input type="submit" class="proceed-btn" name="selesai" value="  VERIFIKASI JIKA PESANAN TELAH SAMPAI "></input>
                                <?php
                                        }
                                        else{
